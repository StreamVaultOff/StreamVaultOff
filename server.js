...
function saveDb(films) {
  fs.writeFileSync(DB_PATH, JSON.stringify(films, null, 2), 'utf8');
}
...
const storage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    ...
    const safeBase = sanitizeFilename(base).slice(0, 80) || 'video';
...
app.post('/api/films', upload.single('video'), (req, res) => {
  if (!req.file) {
  ...
  const title = titleInput || path.basename(req.file.originalname, path.extname(req.file.originalname));
  const film = {
...
app.listen(PORT, () => {
  console.log(`StreamVault server running at http://localhost:${PORT}`);
});
